...
pygame.init()
sound1 = pygame.mixer.Sound('/home/pi/mp3/03bye.wav')
time.sleep(1.)
...
while True:
    ...
    # 번호판을 잘못읽어 쿼리가 실패했을 경우의 오류를 처리하기 위한 try except.
    try:
        # 쿼리해온 값과 현재 시간의 차이를 구함.
        now_int = datetime.now()
        parking_time = now_int - query[0][0]
    except:
        continue

    # 요금을 계산.
    if parking_time.seconds >= 600 or parking_time.days != 0:
        # 10분 이상 6만원.
        pee = 60000
    elif parking_time.days == 0:
        # 초당 100원.
        pee = parking_time.seconds * 100

    # 번호판을 기준으로 DB의 로그 테이블에서 레코드의 출차시간과 요금을 업데이트하고 주차상태 테이블에서는 레코드 삭제.
    db = pymysql.connect(host='AWS 호스팅된 아이피', user='Mysql계정', password='비밀번호', db='데이터베이스', charset='utf8')
    ...
    try:
        ...
        sql = "UPDATE `admin_log` SET `출차시간` = %s, `금액` = %s WHERE `번호판` = %s AND `출차시간` IS NULL;"
        cur.execute(sql, data)
        db.commit()

        sql = "DELETE FROM `parking_status` WHERE `번호판` = %s;"
        cur.execute(sql, chars)
        db.commit()
    ...
    # 처리완료 음성재생하고 차단기 open 신호를 아두이노에 전송.
    sound1.play()
    for i in range(2):
        permission = 'YES'
        if permission == "YES":
